...
export class BrandListComponent implements OnInit {
     ...
     */
    ngOnInit() {
        this._titleService.setTitle( 'Brands' );
        ...
        // Send request to get brands
        this.httpClient.get(environment.apiURL+'/brands'+params)
            .pipe(
                takeUntil(this._unsubscribeAll),
                map (
                    (response) =>{
                        this.dataSource = new MatTableDataSource<any>(response['brands']['data']);
                        this.totalSize = response['total'];
                    }
                ),
                catchError(
                    (error) => {
                        this._sharedService.openSnackBar('There was a problem loading brands.', 'X', 'error', 15000);
    ...
    */
    sortTable(event) {
        ...
        this.httpClient.get(environment.apiURL+'/brands'+params['stringParams'])
            .pipe(
                takeUntil(this._unsubscribeAll),
                map(
                    (response) => {
    ...
    delete(id, index){
        ...
        dialogRef.afterClosed().subscribe(result => {
            if(result == 'confirm'){
                ...
                this.httpClient.delete(environment.apiURL+'/brands/delete/'+id)
                    .pipe(
                        takeUntil(this._unsubscribeAll),
                        map(
                            (response) => {
                                if(response['success']){
                                    ...
                                    this._sharedService.openSnackBar('Brand deleted successfully', 'X', 'success');
                                }
                                this.store.dispatch(new SharedActions.SetIsLoading(false));
                            }
                        ),
                        catchError((error) => {
                            this._sharedService.openSnackBar('Brand not deleted', 'X', 'error', 15000);
                            this.store.dispatch(new SharedActions.SetIsLoading(false));
    ...
    */
    bulkDelete(){
        ...
        const dialogRef = this.dialog.open(DialogComponent, {
            ...
            data: {
                title: 'Delete',
                text: 'Are you sure that you want to delete selected brands?'
            }
        });
        ...
        dialogRef.afterClosed().subscribe(result => {
            if(result == 'confirm'){
                ...
                this.httpClient.post(environment.apiURL+'/brands/delete/bulk', data)
                    .pipe(
                        takeUntil(this._unsubscribeAll),
                        map(
                            (response) => {
                                if(response['success']){
                                    ...
                                    this._sharedService.openSnackBar('Brands deleted successfully', 'X', 'success');
                                }
                                this.store.dispatch(new SharedActions.SetIsLoading(false));
                            }
                        ),
                        catchError((error) => {
                            this._sharedService.openSnackBar('Brands not deleted', 'X', 'error', 15000);
                            this.store.dispatch(new SharedActions.SetIsLoading(false));
